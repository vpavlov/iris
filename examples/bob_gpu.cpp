...
#include <algorithm>
#include <limits>
#include <iris/iris_gpu.h>
#include <iris/memory.h>
#include <iris/logger_gpu.h>
#include <iris/mesh_gpu.h>
#include <iris/comm_rec_gpu.h>
...
main(int argc, char **argv)
{
	...
	client_size = size;
	server_size = size;
x->m_logger->trace("%s %d",__FILE__,__LINE__); MPI_Comm_dup(MPI_COMM_WORLD, &local_comm);
	...
	role = (rank < client_size)?IRIS_ROLE_CLIENT:IRIS_ROLE_SERVER;
     x->m_logger->trace("%s %d",__FILE__,__LINE__); MPI_Comm_split(MPI_COMM_WORLD, role, rank, &local_comm);
	...
	x = new iris_gpu(client_size, server_size, role, local_comm,
