...
void read_gro(char *fname, iris_real **&charges)
{
    ...
    memory::create_2d(charges, ATOMS, 4);
	...
	char **tokens = split(line, 4);
	iris_real x = atof(tokens[0]);
	iris_real y = atof(tokens[1]);
	iris_real z = atof(tokens[2]);
	...
	charges[count][1] = y;
	charges[count][2] = z;
	charges[count][3] = (iris_real) atof(tokens[3]);
	delete [] tokens;
	count++;
    }
...
{
    if(argc < 3) {
	printf("Usage: %s <path-to-charges.dat> <mode>\n", argv[0]);
    ...
    // straightforward way: first ATOMS/client_size charges go to the first
    ...
    x->set_laplacian(IRIS_LAPL_STYLE_PADE, 0, 4);
    x->commit();
	...
	//x->m_mesh->check_fxyz();
	x->m_mesh->dump_bov("phi", x->m_mesh->m_phi);
